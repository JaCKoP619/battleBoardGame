...
void loadSaveMenu()
{
  ...
  while (true)
  {
    ...
    if (input == 'y')
    {
      ...
      std::cout << std::endl
                << "please input turn number" << std::endl;
      std::cin >> turn;
      std::cout << std::endl
                << "please input maximum number of turns" << std::endl;
      std::cin >> maxTurn;
      std::cout << std::endl
                << "please input maximum time of the turn in seconds. When turn is not finished in time game will end." << std::endl;
      std::cin >> turnTIME;
      if (turnTIME > 5)
        turnTIME = 5;
      if (turnTIME < 900)
...
//* display list of units and menu for selection, like in navigateList(); ----------------------------------------------------------TESTED OK
void attackList(int count)
{
  ...
  if (playerTeam == true)
  {
    for (int i = count * 10; i < ((count * 10) + 10); i++)
    {
      if (i < blueUnits.size() && blueUnits[i].getStatus() == true)
        blueUnits[i].info();
    }
  }
  else
  {
    for (int i = count * 10; i < ((count * 10) + 10); i++)
    {
      if (i < redUnits.size() && redUnits[i].getStatus() == true)
        redUnits[i].info();
    }
  }
}
...
bool readUnits(int turnTIME)
{
  ...
  // clear unit vectors for overwriting
  blueUnits.clear();
  redUnits.clear();
  ...
  try
  {
    ...
    if (std::remove(filePath.string().c_str()) == 0)
      std::cout << "Orders recived!" << std::endl;
  }
...
//* menu displays map with marked range of attack, as well as lists potetial targets and allows for selection. --------------------tested ok
void attackMenu()
{
  int size;
  int selectedId;
